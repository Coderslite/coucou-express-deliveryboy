                ...
                children: [
                            ...
                            child: Container(
                              ...
                              child: Row(
                                ...
                                children: [
                                  ...
                                  8.width,
                                  Text(
                                    getBoolAsync(AVAILABLE, defaultValue: true) ? appStore.translate('available') : appStore.translate('not_available'),
                  ...
                  Observer(
                    builder: (_) => SettingItemWidget(
                      ...
                      onTap: () {
                        showConfirmDialog(context, appStore.translate('do_you_want_to_change_the_city'), positiveText: appStore.translate('yes'), negativeText: appStore.translate('no'))
                            .then((value) async {
                          if (value ?? false) {
                            appStore.setLoading(true);

                            await getUserCurrentCity().then((city) async {
                              if (city.validate().isNotEmpty) {
                                await userService.updateDocument(getStringAsync(USER_ID), {
                                  UserKey.city: city,
                                }).then((value) async {
                                  toast('Your current city is now $city');
                                }).catchError((error) {
                  ...
                  SettingItemWidget(
                    ...
                    leading: selectedLanguageDataModel != null ? Image.asset(selectedLanguageDataModel!.flag.validate(), height: 24, width: 24) : null,
                    subTitle: appStore.translate('choose_app_language'),
                    titleTextStyle: primaryTextStyle(),
                    trailing: Text(getSelectedLanguageModel()!.name.validate(), style: primaryTextStyle()),
                    onTap: () async {
                      await StatefulBuilder(builder: (context, setState) {
                        return Scaffold(
                          appBar: appBarWidget(appStore.translate('choose_app_language')),
                  ...
                  SettingItemWidget(
                    ...
                    onTap: () async {
                      bool? res =
                          await showConfirmDialog(context, appStore.translate('do_you_want_to_logout_from_the_app'), positiveText: appStore.translate('yes'), negativeText: appStore.translate('no'));
                      if (res ?? false) {
